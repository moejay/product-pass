...
- Annotation access is optional and requested per origin. On the first image-backed annotation, Product Pass separately explains and requests optional HTTP/HTTPS access because browsers require broad visible-tab permission for reliable screenshots after navigation; annotation injection remains limited to sites the user enables. Custom AI endpoint access is requested when settings are saved. Device flows request only the exact GitHub origins or `https://auth.openai.com/*` and `https://chatgpt.com/*` at runtime from extension contexts; tokens never enter content scripts, page DOM, issue text, AI prompts, URLs, or logs.
...
- Screenshot capture covers only the currently visible viewport, requires the optional HTTP/HTTPS screenshot permission, and may be unavailable on restricted pages or inactive tabs. Screen recording always shows the browser surface chooser, records no audio, and is canceled if the sidebar closes.
